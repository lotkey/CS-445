...
{
  std::string str2;
  for (int i = 0; i < str.length();) { str2 += get_char(str, i); }
  return str2;
}
...
std::string strutil::unescape(const std::string& str)
{
  std::string unescaped;
  for (const auto& c : str) { unescaped += unescape(c); }
  return unescaped;
}

std::string strutil::unescape(char c)
{
  switch (c) {
    case '\n': return "\\n";
    case '\\': return "\\";
